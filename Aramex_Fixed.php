...
class Aramex extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function createOrder($order)
    {
        ...
        if (is_array($replay) && isset($replay[1])) {
            if ($replay[1] == "DOM") {
            ...
            if (isset($replay[2]) && $replay[2] == "error") {
    ...
    private function postAction($method, $post = [])
    {
        try {
            ...
            if (!$order || !$order->getId()) {
                $this->addLog('Order not found with increment ID: ' . $order_id);
                return [__('Order not found'), $method, 'error'];
            }
            ...
            try {
                ...
                if ($auth_call->HasErrors) {
                    ...
                    $this->addLog('Aramex API Error: ' . $errors);
                    return [$errors, $method, 'error'];
                } else {
                    // Get the AWB number from response
                    $awbNumber = $auth_call->Shipments->ProcessedShipment->ID;
                    
                    $data = [
                        'items' => $post['aramex_items'],
                        'comment_text' => "Aramex Shipment Order AWB No. " . $awbNumber . " - Order No. " . $order->getId() .
                            " <a style='color:red;'>Print Label #" . $awbNumber . "</a>",
                        'comment_customer_notify' => false,
                        'is_visible_on_front' => true
                    ];

                    if ($order->canShip() && $post['aramex_return_shipment_creation_date'] == "create") {
                        $shipment = $this->createShipmentWithTracking($order, $data, $awbNumber);
                        
                        if ($shipment && $shipment->getId()) {
                            $this->addLog("Shipment created with ID: " . $shipment->getId());
                            $this->generateAwb($order, $awbNumber);
                            $this->generateSchedulePickup($order);
                            return [__('Shipment created successfully'), $method, 'success'];
                        } else {
                            $this->addLog('Failed to create shipment');
                            return [__('Failed to create shipment'), $method, 'error'];
                        }
                    } else {
                        $this->addLog('Cannot create shipment for the order. Order ID: ' . $order->getId() . ', Can Ship: ' . ($order->canShip() ? 'Yes' : 'No'));
                        return [__('Cannot create shipment for this order'), $method, 'error'];
                    }
                }
            } catch (\Exception $e) {
                $this->addLog('SOAP Exception: ' . $e->getMessage());
                return [$e->getMessage(), $method, 'error'];
            }
        } catch (\Exception $e) {
            $this->addLog('General Exception: ' . $e->getMessage());
            return [$e->getMessage(), $method, 'error'];
        }
    }
    ...
    private function createShipmentWithTracking($order, $data, $awbNumber)
    {
        try {
            ...
            if ($shipment) {
                // Create tracking using TrackFactory instead of direct instantiation
    ...
    private function _saveShipment($shipment)
    {
        try {
            ...
            $this->addLog('Shipment saved successfully');
        } catch (\Exception $e) {
            $this->addLog('Error saving shipment: ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    private function getShipmentDescription($order)
    {
        ...
        // Removed the var_dump and die() which was causing issues
    ...
    public function getClientInfo($orderId)
    {
        ...
        $pin = $this->scopeConfig->getValue(
            'aramex/settings/account_pin',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $storeId
        );
        $entity = $this->scopeConfig->getValue(
